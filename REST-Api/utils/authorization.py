...
def process_firebase_authorization_field(request):
    try:
        authorization_args = authorization_header.parse_args(request)
        authorization_components = authorization_args.get('Authorization')
...
def process_trello_authorization_field(request):
    try:
        authorization_args = authorization_header.parse_args(request)
        authorization_components = authorization_args.get('Authorization').split(",")
