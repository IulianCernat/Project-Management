...
def create_profiles(profile_list, admin_id_token):
	...
	teacher_profiles = profile_list[:6]
	student_profiles = profile_list[6:]
...
def create_projects(teacher_ids):
		...
		for project_id in projects_by_user['created_projects']:
			...
			for _ in range(6):
				possible_scrum_masters = list(
					filter(lambda item: item not in already_added_scrum_masters, users_ids))
...
if __name__ == '__main__':
	...
	created_teachers_ids, created_students_ids, uidList = create_profiles(profiles, admin_id_token)
	...
	add_team_members(projects_with_teams, created_students_ids)
